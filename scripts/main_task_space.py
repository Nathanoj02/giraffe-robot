...
if __name__ == "__main__":
    ...
    pos_final = conf.p_des.copy()
    pitch_des = np.radians(conf.pitch_des)

    frame_id = model.getFrameId(conf.frame_name)

    controller = TaskSpaceController(robot, model, data, ros_pub, q, qd, qdd, pos_final, pitch_des)

    # Ask user to start simulation
    input("Press Enter to start the task space control simulation...")

    q_final, qd_final, logs = controller.simulate()
    ...
    pitch_final_cartesian = np.arcsin(-R_world[2, 2])
    ...
    for i in range(3):
        plt.subplot(4, 1, i+1)
        plt.plot(logs['time'], logs['p'][i], label=f'Actual {labels[i]}', linewidth=3)
        plt.plot(logs['time'], logs['p_des'][i], '--', label=f'Desired {labels[i]}', linewidth=2)
    ...
    # Pitch plot
    plt.subplot(4, 1, 4)
    plt.plot(logs['time'], np.degrees(logs['pitch']), label='Actual', linewidth=3)
    plt.plot(logs['time'], np.degrees(logs['pitch_des']), '--', label='Desired', linewidth=2)
